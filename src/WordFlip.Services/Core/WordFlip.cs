namespace Wordsmith.WordFlip.Services.Core
{
    ...
    public static class WordFlip
    {
        ...
        public static string Flip(ReadOnlySpan<char> sentence)
        {
            ...
            string sanitizedSentence = null;

            unsafe
            {
                fixed (char* ptr = sentence)
                {
                    sanitizedSentence = string.Create(stringLength, (Pointer: new[] { ptr }, OriginalLength: sentence.Length, Length: stringLength), (buffer, sentencePtr) =>
                    {
                        var startPosition = 0;
                        var endPosition = 1;
